...
function topology_wizard()
    ...
    data.converter.vin_min = 36;      % V
    data.converter.vin_max = 72;      % V
    data.converter.vin_nom = [];       % V (optional - computed as midpoint if empty)
    data.converter.vout = 5;           % V
    data.converter.iout = 32;           % A
    data.converter.fsw_khz = 221;      % kHz

    % Converter specs - optional (with defaults)
    data.converter.efficiency = 88;    % percent
    data.converter.vd = 0.7;           % V (diode forward drop)
    data.converter.max_ripple = 25;    % percent
    ...
    data.rec.include_toroidal = false;  % Toroidal-only mode (OM limitation: either/or)
    data.rec.include_all_shapes = true; % Dual search: both toroidal + non-toroidal
    ...
    data.rec.fill_min_pct = 20;   % Hard reject below this fill factor (%)
    data.rec.fill_max_pct = 60;   % Manufacturability warning above this (%)
...
function build_wizard_panel(data)
    ...
    data.chk_include_toroidal = uicontrol('Parent', rec_panel, 'Style', 'checkbox', ...
              ...
              'Value', 0, ...
              'FontSize', 8, ...
              'TooltipString', 'Checked = toroidal cores only. Unchecked = E/ETD/PQ/RM/etc. only. Use "All core shapes" for both.', ...
              'Callback', @cb_include_toroidal);

    uicontrol('Parent', rec_panel, 'Style', 'checkbox', ...
              'String', 'All core shapes', ...
              'Units', 'normalized', ...
              'Position', [0.32 0.62 0.28 0.12], ...
              'Value', 1, ...
              'FontSize', 8, ...
              'TooltipString', 'Search both toroidal AND non-toroidal cores (runs adviser twice, takes longer)', ...
              'Tag', 'chk_all_shapes', ...
              'Callback', @cb_include_all_shapes);
    ...
    % Window fill factor limits
    make_label(rec_panel, 'Min fill %:', [0.02 0.32 0.12 0.10]);
    data.edit_fill_min = uicontrol('Parent', rec_panel, 'Style', 'edit', ...
              'String', num2str(data.rec.fill_min_pct), ...
              'Units', 'normalized', ...
              'Position', [0.14 0.34 0.08 0.08], ...
              'FontSize', 8, ...
              'TooltipString', 'Hard reject recommendations below this fill factor (%)', ...
              'Callback', @cb_fill_min);

    make_label(rec_panel, 'Max fill %:', [0.24 0.32 0.12 0.10]);
    data.edit_fill_max = uicontrol('Parent', rec_panel, 'Style', 'edit', ...
              'String', num2str(data.rec.fill_max_pct), ...
              'Units', 'normalized', ...
              'Position', [0.36 0.34 0.08 0.08], ...
              'FontSize', 8, ...
              'TooltipString', 'Manufacturability warning above this fill factor (%)', ...
              'Callback', @cb_fill_max);

    % Results count label
    data.txt_rec_count = uicontrol('Parent', rec_panel, 'Style', 'text', ...
              'String', 'Results: (awaiting compute)', ...
              'Units', 'normalized', ...
              'Position', [0.02 0.22 0.96 0.10], ...
              'FontSize', 8, ...
              'HorizontalAlignment', 'center', ...
...
function cb_fill_min(src, ~)
    fig = gcbf();
    data = guidata(fig);
    val = round(str2double(get(src, 'String')));
    if isnan(val) || val < 1, val = 1; end
    if val > 90, val = 90; end
    data.rec.fill_min_pct = val;
    set(src, 'String', num2str(val));
    guidata(fig, data);
end

function cb_fill_max(src, ~)
    fig = gcbf();
    data = guidata(fig);
    val = round(str2double(get(src, 'String')));
    if isnan(val) || val < 20, val = 20; end
    if val > 95, val = 95; end
    data.rec.fill_max_pct = val;
    set(src, 'String', num2str(val));
    guidata(fig, data);
end

function cb_include_all_shapes(src, ~)
    fig = gcbf();
    data = guidata(fig);
    data.rec.include_all_shapes = logical(get(src, 'Value'));
    guidata(fig, data);
end
...
function cb_compute_topology(~, ~)
    ...
    try
        ...
        % When neither toroidal-only nor non-toroidal-only is desired,
        % run dual search (both passes) and merge results.
        api_config.adviser_settings.include_all_core_shapes = data.rec.include_all_shapes;
        % Window fill factor limits for filtering recommendations
        api_config.adviser_settings.fill_min_pct = data.rec.fill_min_pct;
        api_config.adviser_settings.fill_max_pct = data.rec.fill_max_pct;
